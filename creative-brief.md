# Creative Brief — Unsubscribe / Suppression

**Source URL (recreate to the letter, then clean it up):** [https://www.foundcashnow.com/unsubscribe](https://www.foundcashnow.com/unsubscribe)
**Notes from tracker:** Legal Page
**Folder:** `pages/www.foundcashnow.com_unsubscribe/`
...
Lets a user remove their email from all marketing lists, or download the suppression list.

This page is a **like-for-like recreation** of `https://www.foundcashnow.com/unsubscribe` — every clause, field, headline, and link from the live page must appear in the rebuild. Pull the exact body copy and structure from the live page, then re-skin it using the design system below. Do not paraphrase legal text; copy it verbatim.

## 2. Content sections (in order)

1. Header — title 'Unsubscribe' (or 'Suppression List Download' for the prizestash variant)
2. 1-paragraph explainer
3. Form: Email (required), confirm email, optional reason dropdown ('Too many emails' / 'Not relevant' / 'Other'), Submit
4. Success panel: 'You've been removed', plus link to re-subscribe
5. For suppression-list-download variant: instead of a form, a download button that serves a hashed suppression list as required by CAN-SPAM/CASL — explainer states the file is SHA-256 hashed emails, updated daily.

> **Sourcing note:** open `https://www.foundcashnow.com/unsubscribe` and copy the actual text into the matching section above. If a section on the live page is missing here, add it; if a section here doesn't exist on the live page, drop it. Goal is exact content parity, cleaner presentation.

## 3. Layout

Single column, 480px max-width centered card.

## 4. Calls to action

Primary 'Unsubscribe' (or 'Download list') button, Push Purple, 48px tall.
...
- [ ] Every piece of copy from `https://www.foundcashnow.com/unsubscribe` is present in `index.html` (verbatim for legal sections).
